...
const TracksSection = () => {
  return (
    <section id="tracks" className="py-16 md:py-20 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
          {tracks.map((track, index) => (
            <motion.div
              ...
              className="glassmorphism p-5 md:p-6 rounded-lg border border-white/10 hover:border-neon-blue/50 transition-all duration-300"
            >
              <div className="text-3xl md:text-4xl mb-3 md:mb-4">{track.icon}</div>
              <h3 className={`text-lg md:text-xl font-cyber mb-2 md:mb-3 text-${track.color}`}>{track.title}</h3>
              <p className="text-gray-300 text-sm md:text-base">{track.description}</p>
            </motion.div>
          ))}
        </div>
